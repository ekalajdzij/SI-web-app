...
function UserCampaignCRUD() {
  ...
  useEffect(() => {
    ...
    const userDataFromStorage = JSON.parse(
      localStorage.getItem("userForCampaign")
    );
    if (userDataFromStorage) {
      ...
      if (
        ...
        setName(
          JSON.parse(localStorage.getItem("campaignData")).find(
            (destinacija) => destinacija.id == localStorage.getItem("campId")
          )?.name
        );
    }
  ...
  const handleSave = async (id) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://fieldlogistics-control.azurewebsites.net/api/user/campaigns/${editableRow}`,
        {
          method: "PUT",
          headers: {
