...
function App() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      (localStorage.getItem("isLoggedInVia2fa") === "true" ||
        localStorage.getItem("isLoggedInVia2fa") === "true" ||
        localStorage.getItem("isLoggedInVia2fa") === true)
    ) {
      setSigned(true);
  ...
  return (
    <div>
      ...
      <Routes>
        <Route
          path="/home"
          element={signedIn ? <Home isSuper={isSuperAdmin} /> : <Error />}
        />
        <Route
          path="/"
          element={
            <Login
              QR={setQRfunction}
              visib={setVisfunction}
              supe={setSuperfunction}
            />
          }
        />
        ...
        <Route
          path="/admin"
          element={isSuperAdmin && signedIn ? <CRUDadmin /> : <Error />}
        />
        <Route
          path="/users"
          element={!isSuperAdmin && signedIn ? <CRUDuser /> : <Error />}
        />
        <Route
          path="/campaign"
          element={!isSuperAdmin && signedIn ? <CampaignView /> : <Error />}
        />
        <Route
          path="/usercamp"
          element={!isSuperAdmin && signedIn ? <UserCampaignCRUD /> : <Error />}
        />
        <Route
          path="/location"
          element={
            !isSuperAdmin && signedIn ? (
              <Locations setGoBack={setGoBack} />
            ) : (
              <Error />
            )
          }
        ...
        <Route
          path="/record"
          element={
            !isSuperAdmin && signedIn ? <Records back={back} /> : <Error />
          }
        />

        <Route
          path="/company"
          element={isSuperAdmin && signedIn ? <Company /> : <Error />}
        />
        <Route
          path="/twofactor"
          element={
