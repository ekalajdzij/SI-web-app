...
import { FaTrash, FaEdit, FaCheck } from 'react-icons/fa'; import "../css/CRUDadmin.css";
...
function CRUDadmin() {
  ...
  const handleEditClick = (id) => {
    ...
    const foundCompany = (companyData).find((comp) => comp.name == foundAdmin.company)
  ...
  const deleteAdmin = async (id) => {
    try {
      ...
      const response = await fetch(`https://fieldlogistics-control.azurewebsites.net/api/admin/${id}`, {
        method: 'DELETE',
        headers: {
  ...
  const handleConfirmEdit = async (id) => {
    try {
      ...
      if (editedData[id].companyId) {//console.log(editedData[id].companyId)
        //console.log(companyData);
        const companyName = (companyData).find(company => company.id == editedData[id].companyId);
        console.log(companyName)


        const updatedAdmins = adminData.map(admin =>
          admin.id === id ? { ...admin, ...updatedAdmin, company: companyName.name } : admin);
        setAdminData(updatedAdmins);
        localStorage.setItem('adminData', JSON.stringify(updatedAdmins));
  ...
  return (
    <div className="table-container">
      ...
      <table>
        ...
        <tbody>
          {adminData.filter((admin) => !admin.isSuperAdmin).map((admin) => (
            <tr key={admin.id}>
              <td>{admin.id}</td>
              <td>
              ...
              <td>
                {editableRow === admin.id ? (
                  <select
                    name="companyId"
                    value={editedData[admin.id]?.companyId || companyData.find(comp => comp.name === admin.company)?.id}
                    onChange={(e) => handleInputChange(e, 'companyId', admin.id)}
                  >
              ...
              <td>
                {editableRow === admin.id ? (
                  <>
                    ...
                    <FaCheck onClick={() => handleConfirmEdit(admin.id)} className="confirmEditBtn" />

                  </>
                ) : (


                  <FaEdit onClick={() => handleEditClick(admin.id)} className="editBtn" />)}

                <FaTrash className="delete-icon" onClick={() => deleteAdmin(admin.id)} />
      ...
      {showModal && <AddAdminModal closeModal={() => setShowModal(false)} companyData={companyData} setAdminData={setAdminData} />}
    </div>
  );
